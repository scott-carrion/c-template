...
static void descsig_handler(int signo, siginfo_t* info, void* vcontext)
{
	...
	fprintf(stderr, "\e[1;31mSent by pid:\e[0m %d\n", info->si_pid);
	fprintf(stderr, "\e[1;31mReal user id of sending process:\e[0m %u\n", info->si_uid);
	fprintf(stderr, "\e[1;31mReported address of faulting instruction:\e[0m 0x%llx\n", (unsigned long long)info->si_addr);
	...
	// If the signal was not any of the above, check the generic signal codes
		...
		switch (info->si_code) {
			...
			case SI_MESGQ:
				...
				break;
			default:
				char* codedesc_tmp = new char [128];
				strcpy(codedesc_tmp, "Unknown signal code (");
				char* sc_tmp = new char [4];
				sprintf(sc_tmp, "%d", info->si_code);
				strcat(codedesc_tmp, sc_tmp);
				strcat(codedesc_tmp, ")");

				strcpy(codedesc, codedesc_tmp);
				//strcpy(codedesc, "Unknown signal code");
				break;
		}
	}

	// Now that we know the signal code, print its description to stderr
	fprintf(stderr, "\e[1;31mSignal code:\e[0m %s", codedesc);
	...
	if (signo == SIGABRT || signo == SIGALRM || signo == SIGBUS || signo == SIGFPE || signo == SIGILL || signo == SIGPIPE || signo == SIGSEGV || signo == SIGXCPU || signo == SIGXFSZ) {
		fprintf(stderr, "\n\e[47m\e[1;31m  *** END DESCSIG SIGNAL REPORT ***  \e[0m\nKilling sending process... ");
	...
	// Otherwise, prompt the user to continue
		...
		while (read(STDIN_FILENO, &buf, 1) < 0) { perror("Internal read error (descsig: continue inquiry)"); }  // Blocking read from stdin, waiting for user answer
